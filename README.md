...
V2.11.10 is a controlled testing release. The email/password implementation is intentionally not presented as the final production identity architecture. Before public release, use a production identity provider/managed authentication design and complete Google/Apple server-side verification.


## Deployment configuration
V2.11.10 requires the Cloudflare Runtime Secret `DONEXT_AUTH_SECRET`. The Wrangler configuration declares it as required and enables `keep_vars = true`. The secret value is never stored in this repository.
